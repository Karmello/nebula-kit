...
metadata: 
  ...
  modified: 2026-08-25T18:44:24.494Z
---
...
**How to apply:** Don't propose merging `props.ts` into `types.ts` or replacing the `annotate-*.js` injection scripts with native JSDoc unless the user raises it again. See also [[project_docs_meta_flattening]] for the props.ts self-containment work this sits alongside — related but separate decision (that one is about cross-component references *within* props.ts, this one is about props.ts vs types.ts as two files).
